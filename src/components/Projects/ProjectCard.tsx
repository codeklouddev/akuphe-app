...
import { convertGoogleDriveUrl } from '@/utils/imageHelpers'

const IconText: React.FC<{ icon: React.FC<React.SVGProps<SVGSVGElement>>; text: string }> = ({ icon: Icon, text }) => (
  <li className="flex gap-2">
    <Icon className="size-[18px] md:size-5 text-accent" />
    ...
    <div className="bg-secondary border-border flex flex-col justify-between rounded-[14px] border p-5">
      <div className="flex items-start justify-between gap-2">
        ...
        <figure className="flex justify-end overflow-hidden">
          <Image
            src={convertGoogleDriveUrl(cover)}
            width={150}
            height={80}
            alt="Project Cover"
